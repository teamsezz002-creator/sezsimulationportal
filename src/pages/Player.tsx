...
export function Player() {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/api/check-installed/${sim.id}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.installed) {
          setVirtualUrl(data.url.startsWith('http') ? data.url : backendUrl + data.url);
          setIsReady(true);
        }
      })
  ...
  const handleDownloadAndInstall = async () => {
    if (sim.sourceType === "uploaded" && sim.storageUrl) {
      ...
      try {
        const backendUrl = import.meta.env.VITE_BACKEND_URL || '';
        const response = await fetch(`${backendUrl}/api/mount-zip`, {
          method: "POST",
        ...
        if (data.url) setVirtualUrl(data.url.startsWith('http') ? data.url : backendUrl + data.url);
