  ...
  const simulateBuildProcess = async () => {
    ...
    if (uploadFile) {
      ...
      try {
        ...
        const buildRes = await fetch(`${backendUrl}/api/build-react`, {
            method: 'POST',
            headers: {
        ...
        while (buildStatus === 'building') {
            ...
            const statusRes = await fetch(`${backendUrl}/api/build-status/${jobId}`, {
                headers: {
        ...
        const downloadRes = await fetch(`${backendUrl}/api/build-download/${jobId}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        });
